  ...
  <div class="wrapper">
    ...
    <el-dialog title="库存维护" :visible.sync="stockDialogVisible">
      ...
      <el-table :data="stockTableData">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button v-if="scope.row.id" size="mini" type="text" @click="editSkuStock(scope.row)">保存</el-button>
            <el-button v-else size="mini" type="text" @click="saveStockInfo(scope.row)">保存</el-button>
  ...
  methods: {
    ...
    editSkuStock(row) {
      this.$confirm("是否修改库存信息?", "提示", {
        ...
        //发送请求
        this.$http({
          url: this.$http.adornApiUrl("/wms/waresku/update"),
          method: "post",
    ...
    editSku(row) {
      ...
      //获取当前sku对应的所有库存
      this.$http({
        url: this.$http.adornApiUrl("/wms/waresku/" + row.skuId),
        method: "get"
      }).then(({ data }) => {
    ...
    getSkusBySpuId(id) {
      ...
      return new Promise((resolve, reject) => {
        this.$http({
          url: this.$http.adornApiUrl("/pms/skuinfo/" + id),
          method: "get"
        }).then(({ data }) => {
