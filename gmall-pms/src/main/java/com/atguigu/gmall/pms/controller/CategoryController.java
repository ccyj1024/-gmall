...
public class CategoryController {
    ...
    @ApiOperation("根据分类级别和父节点id查询")
    @GetMapping
    public Resp<List<CategoryEntity>> queryCategoriesByCidOrLevel(
            @RequestParam(value = "level",defaultValue = "0") Integer level,
            @RequestParam(value = "parentCid",required = false)Long parentCid){
        List<CategoryEntity> categoryEntities = categoryService.queryCategoriesByCidOrLevel(level,parentCid);
